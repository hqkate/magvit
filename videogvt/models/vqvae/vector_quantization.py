...
class VQEmbedding(nn.Embedding):
    ...
    def __init__(self, K, D, ema=True, decay=0.99, restart_unused_codes=True, eps=1e-5):
        ...
        if self.ema:
            ...
            _ = [
                ms.Parameter(p, name="w2", requires_grad=False)
                for p in self.trainable_params()
            ]

            # padding index is not updated by EMA
            self.weight = ms.Parameter(
                ms.Tensor(np.zeros((K + 1, D)), ms.float32),
                name="weight",
                requires_grad=False,
            )
            self.cluster_size_ema = ms.Parameter(
                ms.Tensor(np.zeros(([1, 256])), ms.float32),
                name="w",
                requires_grad=False,
            )
            # self.register_buffer('cluster_size_ema', ops.zeros(K))
            self.embed_ema = ms.Parameter(
    ...
    def compute_distances(self, inputs):
        ...
        inputs_norm_sq = ops.sum(inputs_flat.pow(2.0), dim=1, keepdim=True)
        codebook_t_norm_sq = ops.sum(codebook_t.pow(2.0), dim=0, keepdim=True)
        distances = ops.addmm(
    ...
    def _update_buffers(self, vectors, idxs):
        ...
        one_hot_idxs = ops.tensor_scatter_elements(
        ...
        if self.restart_unused_codes:
            ...
            n_vectors = vectors.shape[0]
            _vectors_random = vectors[ops.randperm(n_vectors)][:K]
            ...
            self.cluster_size_ema = self.cluster_size_ema + (
                ops.ones_like(self.cluster_size_ema) * (1 - usage).view(-1)
            )
    ...
    def compute_commitment_loss(self, x, quant_list):
        ...
        for idx, quant in enumerate(quant_list):
            partial_loss = (x - ops.stop_gradient(quant)).pow(2.0).mean()
            loss_list.append(partial_loss)

        commitment_loss = ops.mean(ops.stack(loss_list))
