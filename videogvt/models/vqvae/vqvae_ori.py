...
class EuclideanCodebook(nn.Cell):
    def __init__(
        ...
        self.cluster_size = ms.Parameter(
            ops.zeros(num_codebooks, codebook_size), requires_grad=False
        )
        ...
        self.codebook_mean_needs_init = ms.Parameter(
            ms.Tensor([True]), requires_grad=False
        )
        self.codebook_mean = ms.Parameter(
            ms.numpy.empty(num_codebooks, 1, dim), requires_grad=False
        )
        self.codebook_variance_needs_init = ms.Parameter(
            ms.Tensor([True]), requires_grad=False
        )
        self.codebook_variance = ms.Parameter(
            ms.numpy.empty(num_codebooks, 1, dim), requires_grad=False
        )
    ...
    def update_with_decay(self, buffer_name, new_value, decay):
        ...
        if needs_init:
            ...
            setattr(
                self,
                buffer_name + "_needs_init",
                ms.Parameter(ms.Tensor([False]), requires_grad=False),
            )
...
class VectorQuantize(nn.Cell):
    ...
    def construct(
        ...
        if exists(mask):
            quantize = ops.where(rearrange(mask, "... -> ... 1"), quantize, orig_input)
