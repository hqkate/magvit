...
from typing import Any, Dict, Tuple, Type, Union, Sequence, Optional, Callable
...
def l2_normalize(x, axis=None, epsilon=1e-12):
    ...
    x_inv_norm = ops.rsqrt(ops.max(square_sum, epsilon))
    return ops.matmul(x, x_inv_norm)
...
def l2norm(t):
    ...
    return ops.L2Normalize(axis=-1, epsilon=1e-12)(t)


def cdist(x, y):
    x2 = reduce(x**2, "b n d -> b n", "sum")
    y2 = reduce(y**2, "b n d -> b n", "sum")
    xy = ops.einsum("b i d, b j d -> b i j", x, y) * -2
    return (
...
def sample_multinomial(total_count, probs):
    ...
    sample = ops.empty_like(probs, dtype=ms.Tensor)

    for i, p in enumerate(probs):
        s = ops.binomial(total_count, p / remainder)
        sample[i] = s
        total_count -= s
        remainder -= p
...
def all_gather_sizes(x, dim):
    size = ms.Tensor(x.shape[dim], dtype=ms.Tensor)
    all_sizes = [ops.empty_like(size) for _ in range(distributed.get_world_size())]
...
def sample_vectors_distributed(local_samples, num):
    ...
    if rank == 0:
        ...
        samples_per_rank = ops.empty_like(all_num_samples)
...
def kmeans(
    ...
    for _ in range(num_iters):
        ...
        means = ops.where(rearrange(zero_mask, "... -> ... 1"), means, new_means)
...
def orthogonal_loss_fn(t):
    ...
    cosine_sim = ops.einsum("h i d, h j d -> h i j", normed_codes, normed_codes)
...
class EuclideanCodebook(nn.Cell):
    def __init__(
        ...
        self.kmeans_all_reduce_fn = ops.AllReduce() if use_ddp and sync_kmeans else noop
        self.all_reduce_fn = ops.AllReduce() if use_ddp else noop

        # self.register_buffer('initted', torch.Tensor([not kmeans_init]))
        # self.register_buffer('cluster_size', torch.zeros(num_codebooks, codebook_size))
        # self.register_buffer('embed_avg', embed.clone())

        self.initted = ms.Parameter(ms.Tensor([not kmeans_init]), requires_grad=False)
        self.cluster_size = ms.Parameter(ops.zeros(num_codebooks, codebook_size), requires_grad=False)
        self.embed_avg = ms.Parameter(embed.copy(), requires_grad=False)

        self.learnable_codebook = learnable_codebook
        if learnable_codebook:
            self.embed = ms.Parameter(embed)
        else:
            # self.register_buffer('embed', embed)
            self.embed = ms.Parameter(embed, requires_grad=False)
        ...
        self.codebook_mean_needs_init = ms.Parameter(ms.Tensor([True]), requires_grad=False)
        self.codebook_mean = ms.Parameter(ms.numpy.empty(num_codebooks, 1, dim), requires_grad=False)
        self.codebook_variance_needs_init = ms.Parameter(ms.Tensor([True]), requires_grad=False)
        self.codebook_variance = ms.Parameter(ms.numpy.empty(num_codebooks, 1, dim), requires_grad=False)
    ...
    def init_embed_(self, data, mask = None):
        ...
        self.embed.data = embed.copy()
        self.embed_avg.data = embed_sum.copy()
        self.cluster_size.data = cluster_size.copy()
        self.initted.data = ms.Tensor([True])
    ...
    def update_with_decay(self, buffer_name, new_value, decay):
        ...
        if needs_init:
            # self.register_buffer(buffer_name + "_needs_init", ms.Tensor([False]))
            setattr(self, buffer_name + "_needs_init", ms.Parameter(ms.Tensor([False]), requires_grad=False))

        if not exists(old_value) or needs_init:
            # self.register_buffer(buffer_name, new_value.detach())
            setattr(self, buffer_name, ms.Parameter(new_value, requires_grad=False))

            return

        value = old_value * decay + new_value.detach() * (1 - decay)
        # self.register_buffer(buffer_name, value)
        setattr(self, buffer_name, ms.Parameter(value, requires_grad=False))

    @ms.jit.ignore
    def update_affine(self, data, embed, mask = None):
        assert self.affine_param

        var_fn = partial(ops.var, unbiased = False)
        ...
        num_vectors = ms.Tensor([num_vectors], dtype = dtype)
        ops.AllReduce()(num_vectors)

        # calculate distributed mean

        batch_sum = reduce(data, 'h n d -> h 1 d', 'sum')
        ops.AllReduce()(batch_sum)
...
class CosineSimCodebook(nn.Cell):
    ...
    def construct(
        ...
        if self.training and self.ema_update and not freeze_codebook:
            ...
            self.embed.data = l2norm(embed_normalized)
            self.expire_codes_(x)
...
class VectorQuantize(nn.Cell):
    def __init__(
        ...
        self.project_in = (
            nn.Dense(dim, codebook_input_dim) if requires_projection else nn.Identity()
        )
        self.project_out = (
            nn.Dense(codebook_input_dim, dim) if requires_projection else nn.Identity()
        )
    ...
    def construct(
        ...
        shape, heads, is_multiheaded, codebook_size, return_loss = (
            x.shape,
            self.heads,
            self.heads > 1,
            self.codebook_size,
        ...
        if should_inplace_optimize and self.training and not freeze_codebook:
            ...
            if exists(mask):
                loss = ops.mse_loss(quantize, x, reduction="none")
                ...
                loss = ops.mse_loss(quantize, x.detach())
        ...
        loss = ms.Tensor([0.0], requires_grad=self.training)
        ...
        if self.training:
            ...
            if self.has_codebook_orthogonal_loss:
                ...
                if (
                    ...
                    rand_ids = ops.randperm(num_codes)[
